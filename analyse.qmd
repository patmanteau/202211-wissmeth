...
logRegPlot <- ggplot(fram, aes(x=GLUCOSE, y=CVDNUM, fill=CVDNUM)) +
  geom_jitter(alpha=0.2, height=0.3) +
  stat_smooth(method = "glm", se = FALSE, method.args = list(family = binomial), formula = CVDNUM ~ GLUCOSE) +
  theme_minimal()
logRegPlot
...
logRegPlot <- ggplot(rbind(withCVD, withoutCVD), aes(x = GLUCOSE)) +
  geom_histogram(data = withCVD, aes(y = 1-..density..), fill = "#4477aa", colour = "#4477aa", alpha = 0.4) +
  geom_histogram(data = withoutCVD, aes(y = -1+..density..), fill = "#ee6677", colour = "#ee6677", alpha = 0.4) + 
  coord_trans(x="log10") + 
  theme_minimal()
logRegPlot
